    ...
    public class Console : MonoBehaviour
    {
        ...
        void ReportMeshCombiner(MeshCombiner meshCombiner, bool foundText = false)
        {
            Log((foundText ? "Found MCS -> " : "") + meshCombiner.name + " (" + (meshCombiner.combined ? "*color-green#Combined" : "*color-blue#Uncombined" ) + ")" + " -> Cell Size " + meshCombiner.cellSize + (meshCombiner.searchOptions.useMaxBoundsFactor ? " | Max Bounds Factor " + meshCombiner.searchOptions.maxBoundsFactor : "")
                    ...
                    + (meshCombiner.searchOptions.useVertexInputLimitLod ? " | Vertex Input Limit Lod " + (meshCombiner.searchOptions.useVertexInputLimitLod ? meshCombiner.searchOptions.vertexInputLimitLod : 65534) : ""), 0, null, meshCombiner);
        }
        ...
        void ExecuteCommand(string cmd)
        {
            ...
            {
                GameObject go = Methods.Find<GameObject>(selectGO, cmd.Replace("show ", ""));
                if (go != null) { go.SetActive(true); log.commandType = 2; }
            }
            else if (cmd == "show")
            {
            ...
            else if (cmd == "vertex input limit lod off")
            {
                if (selectedMeshCombiner != null)
                {
            ...
            else if (cmd.Contains("vertex input limit lod "))
            {
                int vertexInputLimitLod;
                int.TryParse(cmd.Replace("vertex input limit lod ", ""), out vertexInputLimitLod);
                if (vertexInputLimitLod < 1)
                {
                    Log("vertex input limit needs to be bigger than 1");
                    return;
                }
                if (selectedMeshCombiner != null)
                {
                    ...
                    selectedMeshCombiner.searchOptions.vertexInputLimitLod = vertexInputLimitLod;
                    selectedMeshCombiner.AddObjectsAutomatically();
                    if (combineAutomatic) selectedMeshCombiner.CombineAll();
                    ReportMeshCombiner(selectedMeshCombiner);
                    log.commandType = 2;
        ...
        void CD(LogEntry log, string name)
        {
            ...
            GameObject go = Methods.Find<GameObject>(selectGO, name);
